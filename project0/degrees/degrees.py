...
def shortest_path(source, target):
    ...
    # Traverse
    while True:
        # If there aren't any nodes left to explore
        if frontier.empty():
            return None
        # Expand nodes (while checking for circularity)
        for option in neighbors_for_person(cnode.state[1]):
            if option not in explored:
                frontier.add(Node(state=option, parent=cnode))
            # Check for goal state
            if option[1] == target:
                path = [option]
                # Backtracking
                while cnode.state[1] != source:
